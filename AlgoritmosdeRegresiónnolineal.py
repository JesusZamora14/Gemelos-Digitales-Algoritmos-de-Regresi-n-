...
def plotdata(t,x1,x2,xo):
    ...
    plt.plot(t,xo,linewidth = 0.5, markersize = 3, 
...
def biostatistics(Estimate,cov,xo,xa):
    ...
    df = pd.DataFrame(list(zip(Parameter,Estimate,SE,MoE,CI95,
                               pvalue)),
                        columns = ['Parameter','Estimate','SE','MoE','CI95',
                                                   'pvalue'])
...
def plotresults(t,xo,xa,S):
    plt.rcParams.update({'font.size':11})
    fig = plt.figure(figsize = (8,4))
    plt.plot(t,xo,linestyle = 'none', marker = 'x',
             color = [0.38, 0.89, 0.85], label = '$x_o(t)$')
    plt.plot(t,xa,'-',marker = '.', color = [0.38, 0.60, 0.89], 
             label = '$x_a(t)$')
    plt.xlabel('$t$ $[months]$')
    plt.ylabel('x_i(t)$ $[cells]$')
    plt.xlim(0.72)
    ...
    #namepng = 'python_results_' + sigmoidal + '.png'
